...
const fromClientId = 'fromClientId'
const toClientId = 'toClientId'
const userConfig = myConfig;
    ...
    if (ui.useWorker) {
        ...
        human.detect(input, userConfig).then((result) => {
            if (result.error) {
                ...
                if (ui.streamData) {
                    WebsocketClient.sendData(result, toClientId)
                }
...
function setupMenu() {
    ...
    menu.addBool('Stream data to websocket server', ui, 'streamData');
    menu.addHTML('<hr style="min-width: 200px; border-style: inset; border-color: dimgray">');
...
async function main() {
    ...
    if (ui.streamData) {
        ...
        const sid = await WebsocketClient.connect();
        if (sid) {
            await WebsocketClient.joinRoom(fromClientId)
            status('Connected: Client id = ' + fromClientId);
        } else {
            log('Cannot connect to websocket server')
        }
    }
